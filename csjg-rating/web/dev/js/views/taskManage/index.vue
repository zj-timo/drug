...
<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">我的任务管理</span>
      <div class="col"></div>
    </div>
    ...
    <div class="plr-11">
      ...
      <div class="control-group">
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.supervisionMode" :class="{'input-select-on': params.supervisionMode!='' && params.supervisionMode!=null}">
                <option value="">请选择</option>
                <option v-for="item in ratingSupervisionModes" :value="item.id">{{item.name}}</option>
              </select>
            </div>
          </div>
        </div>
       ...
       <div class="control-group">
        <div class="col">
          <div class="row">
            <span class="col w-100">选择时间:</span>
            <div class="row-auto">
              <div class="col">
                <datepicker 
                  :readonly="true" 
      ...
      <div class="row mb-10">
        <div class="table-header">
          <div class="table-name">我的任务管理列表</div>
        </div>
        <!-- table -->
          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in list" v-if="list && list.length!=0">
                ...
                <td>
                  <span>
                    <a href="javascript:;" v-show="item.supervisionMode==1" @click="clickRatingInfo(item.pk)">查看</a>
                  </span>
                  <span v-show="item.supervisionMode==1">|</span>
                  <span>
                    <a href="javascript:;" v-show="item.supervisionMode==1"  @click="clickRatingEdit(item.pk,item.state)" >现场评定</a>
                  </span> 
                  <span>
                    <a href="javascript:;" v-show="item.supervisionMode==2"  @click="clickRegistrationInfo(item.pk)" >查看</a>
                  </span>
                  <span v-show="item.supervisionMode==2">|</span>
                  <span>
                    <a href="javascript:;" v-show="item.supervisionMode==2" @click="clickRegistrationEdit(item.pk,item.state)">现场巡查</a>
                  </span>
                </td>
              </tr>
  ...
  export default {
    ...
    data() {
     ...
     route:{
      data:function(transition){
        if (transition.from.name == "rating-registration-info"  || transition.from.name == "rating-registration-edit"  || transition.from.name == "patrol-registration-info" ||  transition.from.name == "patrol-registration-edit") {
          this.getStorage();
        }
      }
    },
    methods:{ 
     tempSearch(type) {
      ...
      clickRatingInfo(pk) {
        this.setStorage();
        this.$router.go({name:'rating-registration-info', params:{pk:pk}});
      },

      clickRatingEdit(pk,state) {
        this.setStorage();
        this.$router.go({name:'rating-registration-edit', params:{pk:pk,state:state}});
      },

      clickRegistrationInfo(pk) {
        this.setStorage();
        this.$router.go({name:'patrol-registration-info', params:{pk:pk}});
      },

      clickRegistrationEdit(pk,state) {
        this.setStorage();
        this.$router.go({name:'patrol-registration-edit', params:{pk:pk,state:state}});
      },
    },
