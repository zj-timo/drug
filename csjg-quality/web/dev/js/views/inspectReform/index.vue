<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">整改信息管理</span>
    </div>
    <div class="plr-11">
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.inspectionType" :class="{'input-select-on': params.inspectionType!='' && params.inspectionType!=null}">
                <option value="">请选择</option>
                <option v-for="item in qualityInspectionTypes" :value="item.id">
                  {{item.name}}
                </option>
              </select>
            </div>
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.state" :class="{'input-select-on': params.state!='' && params.state!=null}">
                <option value="">请选择</option>
                <option v-for="item in inspectStates" :value="item.id">{{item.name}}</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div class="row mb-10">
        <div class="table-header">
          <div class="table-name">整改信息列表</div>
        </div>
        <!-- table -->
              ...
              <tr v-for="item in list" v-if="list && list.length!=0">
                ...
                <td>
                  <span v-if="item.state != 1">
                    <a href="javascript:;" @click="clickEdit(item.pk)">整改</a>
                  </span>
                  <span v-else>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      industryArea: [],
      fiveYears: fiveYears,
      inspectStates: inspectStates
    }
  },

  route: {
    data: function(transition) {
      if(transition.from.name == 'reform-info-edit' || transition.from.name == 'reform-info-info') {
        this.getStorage();
      }
    }
  },
  ...
  methods: {
    ...
    clickEdit(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-reform-edit', params:{pk:pk}});
    },

    clickInfo(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-reform-info', params:{pk:pk}});
    }
  },
  ...
  ready() {
    //检查类型
    this.model.getDataDictionary({ typeId: "qualityInspectionType" }).then(res => {
