<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">任务受理</span>
    </div>
    <div class="plr-11">
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.inspectionType" :class="{'input-select-on': params.inspectionType!='' && params.inspectionType!=null}">
                <option value="">请选择</option>
                <option v-for="item in qualityInspectionTypes2" :value="item.id">
                  {{item.name}}
                </option>
              </select>
            </div>
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.state" :class="{'input-select-on': params.state!='' && params.state!=null}">
                <option value="">请选择</option>
                <option v-for="item in planStates" :value="item.id">
                  {{item.name}}
                </option>
              </select>
            </div>
      ...
      <div class="row mb-10">
        <div class="table-header">
          <div class="table-name">任务受理列表</div>
        </div>
        <!-- table -->
              ...
              <tr v-for="item in list" v-if="list && list.length!=0">
                ...
                <td>
                  <span v-if="item.state==1">
                    <a href="javascript:;" @click="clickEdit(item.pk)">受理</a>
                  </span>
                  <span v-else>
...
export default {
  ...
  route: {
    data: function(transition) {
      if(transition.from.name == 'inspect-task-edit' || transition.from.name == 'inspect-task-info') {
        this.getStorage();
      }
    }
  },
  methods: {
    ...
    clickEdit(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-task-edit', params:{pk:pk}});
    },

    clickInfo(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-task-info', params:{pk:pk}});
    }
  },
