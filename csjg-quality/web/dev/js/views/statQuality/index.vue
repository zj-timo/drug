<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">统计分析>质量认证</span>
      <div class="col"></div>
      <div class="col-auto">
          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in list" v-if="list && list.length!=0">
                ...
                <td>{{item.companyProblem}}</td>
                <td>{{item.rectificationTime}}</td>
...
export default {
  ...
  methods: {
    ...
    downExcel() {
      ...
      window.open('/csjg-sys/qualityCount/qualityExcel?' + arr.join('&'));
    },
    tempSearch(type) {
