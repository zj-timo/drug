...
<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">年度检查对象管理</span>
      <div class="col"></div>
      <div class="col-auto">
        <input class="btn btn-info" type="button" @click="clickNew()" value="新增">
      </div>
    </div>
    ...
    <div class="plr-11">
      <div class="control-group">
        ...
        <div class="col">
         ...
         <div class="col">
          <div class="row">
            <span class="col w-130">企业分类:</span>
            <div class="col">
              <select class="input full input-select" v-model="params.industryOwned" :class="{'input-select-on': params.industryOwned!='' && params.industryOwned!=null}">
                ...
                <option v-for="item in companyClassAll" :value="item.id">{{item.name}}</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div class="control-group">
        <div class="col">
          <div class="row">
            <span class="col w-100">所属区域:</span>
            <div class="col">
  ...
  export default {
    ...
    route: {
      data: function(transition) {
        if(transition.from.name == 'inspect-obj-new') {
          this.getStorage();
        }
      }
    },
    methods:{ 
      downExcel(){
        ...
        window.open('/csjg-sys/qualityObject/exportExcelCompany?'+arr.join('&'));
      ...
      clickNew() {
        this.setStorage();
        this.$router.go({name: 'inspect-obj-new'});
      }
    },
    ...
    ready() {
      this.model.dataList({typeId:"companyClass"}).then((res) => {
        //企业分类
        this.companyClass = res.data.result;
      })
      this.model.dataList({typeId:"companyClassAll"}).then((res) => {
        //企业分类列表查询
       this.companyClassAll = res.data.result;
      })
