<template>
  <div class="main-content" v-el:main_content>
    <div class="row-auto content-header">
      ...
      <span class="header-title">专项检查信息管理</span>
    </div>
    <div class="plr-11">
          ...
          <div class="row">
            ...
            <div class="col">
              <select class="input full input-select" v-model="params.inspectionType" :class="{'input-select-on': params.inspectionType!='' && params.inspectionType!=null}">
                <option value="">请选择</option>
                <option v-for="item in qualityInspectionTypes2" :value="item.id">{{item.name}}</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div class="row mb-10">
        <div class="table-header">
          <div class="table-name">专项检查列表</div>
        </div>
        <!-- table -->
          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in list" v-if="list && list.length!=0">
                ...
                <td>{{item.inspectionType | filter_map_key qualityInspectionTypes2}}</td>
                <td>{{item.state | filter_map_key reformInspect}}</td>
                <td>{{item.startTime}}</td>
                <td>{{item.endTime}}</td>
                <td>
                  <span v-if="item.state==1">
                    <a href="javascript:;" @click="clickEdit(item.pk)">登记</a>
                  </span>
                  <span v-else>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      industryArea: [],
      fiveYears: fiveYears,
      reformInspect: reformInspect
    }
  },

  route: {
    data: function(transition) {
      if(transition.from.name == 'inspect-supervise-edit' || transition.from.name == 'inspect-supervise-info') {
        this.getStorage();
      }
    }
  },
  ...
  methods: {
    ...
    clickEdit(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-supervise-edit', params:{pk:pk}});
    },

    clickInfo(pk) {
      this.setStorage();
      this.$router.go({name:'inspect-supervise-info', params:{pk:pk}});
    }
  },
