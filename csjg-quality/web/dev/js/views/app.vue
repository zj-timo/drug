	...
	export default {
		...
		route:{
	        ...
	        return
				...
				arr = this.$local_store.get('qualityMenus');
				...
				this.qualityMenus.forEach((item) => {
		...
		methods: {
			getMenuList() {
        this.model.menuList({sysPk: 'csjg-quality'}).then(res => {
          this.qualityMenus = res.data.result;
          this.$local_store.set('qualityMenus', res.data.result);
        })
      },
